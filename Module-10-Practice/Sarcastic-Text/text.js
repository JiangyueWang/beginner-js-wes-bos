const textArea = document.querySelector('[name="text"]');
const result = document.querySelector('.result');
const filterInputs = Array.from(document.querySelectorAll('[name="filter"]'));

/* eslint-disable */
const funkyLetters = {
    '-': '₋', '!': 'ᵎ', '?': 'ˀ', '(': '⁽', ')': '₎', '+': '⁺', '=': '₌', '0': '⁰', '1': '₁', '2': '²', '4': '₄', '5': '₅', '6': '₆', '7': '⁷', '8': '⁸', '9': '⁹', a: 'ᵃ', A: 'ᴬ', B: 'ᴮ', b: 'ᵦ', C: '𝒸', d: 'ᵈ', D: 'ᴰ', e: 'ₑ', E: 'ᴱ', f: '𝒻', F: 'ᶠ', g: 'ᵍ', G: 'ᴳ', h: 'ʰ', H: 'ₕ', I: 'ᵢ', i: 'ᵢ', j: 'ʲ', J: 'ᴶ', K: 'ₖ', k: 'ₖ', l: 'ˡ', L: 'ᴸ', m: 'ᵐ', M: 'ₘ', n: 'ₙ', N: 'ᴺ', o: 'ᵒ', O: 'ᴼ', p: 'ᵖ', P: 'ᴾ', Q: 'ᵠ', q: 'ᑫ', r: 'ʳ', R: 'ᵣ', S: 'ˢ', s: 'ˢ', t: 'ᵗ', T: 'ₜ', u: 'ᵘ', U: 'ᵤ', v: 'ᵛ', V: 'ᵥ', w: '𝓌', W: 'ʷ', x: 'ˣ', X: 'ˣ', y: 'y', Y: 'Y', z: '𝓏', Z: 'ᶻ' };
  /* eslint-enable */

const filters = {
    /** method 1 */
    // sarcastic: function(letter, index) {
    //     if(index % 2 === 0) {
    //         return letter.toUpperCase();
    //     } else {
    //         return letter;
    //     }
    // },
    /** method 2 */
    sarcastic: (letter, index) => {
        return (index % 2 === 0) ? letter.toUpperCase() : letter;
    },
    funky: (letter) => {
        // first check if there is a funcky letter for this 
        let funckyLetter = funkyLetters[letter];
        if (funckyLetter) {
            return funckyLetter;
        }
        // if there is not, check if is a lowercase version
        funckyLetter = funkyLetters[letter.toLowerCase()];
        // if there is nothing, return the regular letter
        return letter;
    },
    unable: function (letter) {
        const random = Math.floor(Math.random() * 3);
        if (letter === ' ' && random === 2) {
          return '...';
        }
        return letter;
    }

};

function transformText(text) {
    // grabbing the value from the filter -> identify which filter has been selected  
        // method 1
    // const filter = document.querySelector('[name="filter"]:checked').value;
        // method 2
    const filterValue = filterInputs
                    .find(input => input.checked) // find the input that has been selected
                    .value; // get the value of the input that has been selected 
    
    // take the text, and loop over each letter
    const mod = Array.from(text).map(filters[filterValue]);
    result.textContent = mod.join("");
 }

filterInputs.forEach((input) => {
    input.addEventListener('input', () => {transformText(textArea.value)})
});
textArea.addEventListener('input', e => transformText(e.target.value));